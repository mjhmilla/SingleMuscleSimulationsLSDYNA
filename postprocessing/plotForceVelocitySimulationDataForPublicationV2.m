...
%                  min(plotSettings(1).yLim)+0.45*(diff(plotSettings(1).yLim)),...
%                 sprintf('SBL1996: %s: %1.2f%s','$$v^M_{max}',vmaxExp,'\ell^M_o/s$$'),...
%                 'FontSize',6,...
...
%             end
        ...
        if(contains(simulationDirectoryName,simMetaData.fileNameSubMaxActEnd))
            ...
            hold on;            
            plot([1,1].*(vmaxSubMax),[0,0.7],'-k','HandleVisibility','off');
            hold on;
            plot([vceMax,vmaxSubMax],[0.65,0.65],'-k','HandleVisibility','off');
            hold on;
            dx = abs(vceMax-vmaxSubMax).*0.05;
            plot([vceMax+2*dx],[0.65],'<k',...
                'MarkerSize',4,...
                'MarkerFaceColor',[0,0,0],...
                'HandleVisibility','off');
            hold on;
            plot([vmaxSubMax-2*dx],[0.65],'>k',...
                'MarkerSize',4,...
                'MarkerFaceColor',[0,0,0],...
                'HandleVisibility','off');
            hold on;            
            text((vmaxSubMax+vceMax)*0.5,0.55,...
                 sprintf('%1.2f%s',abs(vceMax-vmaxSubMax),'$$v^M_o$$'),...
                 'HorizontalAlignment','center',...
                 'VerticalAlignment','middle',...
                 'FontSize',6);
            hold on;
            th=text(vceMax+dx,0.7,sprintf('%1.2f%s',vceMax,'$$v^M_o$$'),...
                 'HorizontalAlignment','left',...
                 'VerticalAlignment','middle',...
                 'FontSize',6);
            th.Rotation=90;
            hold on;
            th=text(vmaxSubMax+dx,0.7,sprintf('%1.2f%s',vmaxSubMax,'$$v^M_o$$'),...
                 'HorizontalAlignment','left',...
                 'VerticalAlignment','middle',...
                 'FontSize',6);
            th.Rotation=90;
        ...
        if(contains(simulationDirectoryName,simMetaData.fileNameMaxActEnd))
            ...
            for indexData=1:1:size(dataExp,1)
             ...
             figure(figH);
             subplot('Position',subplotFv);
             text(-1,1.05,sprintf('RMSE Conc.\n%1.3e',errRmseC),...
                 'HorizontalAlignment','left',...
                 'VerticalAlignment','middle',...
                 'FontSize',6);
             hold on;

             text( 1,1.05,sprintf('RMSE Ecc.\n%1.3e',errRmseE),...
                 'HorizontalAlignment','right',...
                 'VerticalAlignment','middle',...
                 'FontSize',6);
             hold on;
        ...
        if(contains(simulationDirectoryName,simMetaData.fileNameSubMaxActEnd))
            ...
            [lgdH,lgdIcons, lgdPlots, lgdTxt]=...
                legend('Location','NorthWest',...
                        ...
                        'NumColumns',2,'FontSize',fontSizeLegend); 
            ...
            legend boxoff;
%             text(max(plotSettings(1).xLim),...
%                 plotSettings(1).yLim(1,1) + 0.35*diff(plotSettings(1).yLim),...
%                 sprintf('%s %s: %1.2f%s',...
...
if(contains(simulationDirectoryName,simMetaData.fileNameSubMaxActEnd)==1 ...
        && contains(lsdynaMuscleUniform.name,'umat43')==1)
%if(flag_addReferenceData==1)
    ...
    for indexCol = 1:1:length(plotColumnsToFormat)
        ...
        yyaxis right;
            ...
            hT = text(plotSettings(4).xLim(1,2)+dx,...
                 plotSettings(4).yLim(1,1)+0.25*diff(plotSettings(4).yLim),...
        ...
        yyaxis left;
            ...
            legend boxoff;
        yyaxis right;
        hT = text(plotSettings(5).xLim(1,2)+dx,...
             plotSettings(5).yLim(1,1)+0.25*diff(plotSettings(5).yLim),...
            'Norm. Length ($$(\ell^P-\ell^T_s)/\ell^{M}_o$$)',...
            'HorizontalAlignment','left',...
            'VerticalAlignment','top',...
            'FontSize',8,...
