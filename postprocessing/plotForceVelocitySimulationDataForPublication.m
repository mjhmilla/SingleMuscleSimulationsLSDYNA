...
if(flag_addReferenceData==1)
    ...
    idxSample=1;
    dataFvSample  =zeros(numberHL1997Total,2);    
    for idxCol = 2:1:3
        ...
        for idx=idxStart:1:idxEnd
            ...
            if( vSign > 0)
                ...
                dataFvSample(idxSample,1) = ...
                    dataFvSample(idxSample,1) ./ (optimalFiberLength);

                idxRamp = find(dataHL1997Force(idx).x(:,1)>1.5);
                [dataFvSample(idxSample,2), idxSampleTime] = ...
                    max(dataHL1997Force(idx).y(idxRamp,1)./fisoHL1997);
                ...
                idxRamp = find(dataHL1997Force(idx).x(:,1)>1.5);
                [dataFvSample(idxSample,2), idxSampleTime] = ...
                    min(dataHL1997Force(idx).y(idxRamp,1)./fisoHL1997);

                idxSampleTime = idxSampleTime+idxRamp(1,1)-1;                
                idxSample=idxSample+1;
            end
            plot( dataHL1997Force(idx).x(idxSampleTime,1),...
                  dataHL1997Force(idx).y(idxSampleTime,1)./fisoHL1997,...
                  'o',...
                  'Color',expColor,...
    ...
    plot( dataFvSample(:,1)./vmaxSBL1996,...
          dataFvSample(:,2),...
          '-',...
          'Color',expColorB,...
          'MarkerFaceColor',expColorB,...
          'LineWidth',lineWidthModel,...
          'DisplayName',labelHL1997,...
          'HandleVisibility','on');   
    hold on;
    ...
    text(max(plotSettings(1).xLim),...
        ...
        sprintf('%s: %1.2f%s','$$v^M_{max}',vmaxSBL1996,'\ell^M_o/s$$'),...
        'FontSize',6,...
