...
%
%%
function curveValues = calcQuadraticBezierYFcnXCurveSampleVector(curveParams, npts, domain)
%%
...
for k=1:1:length(x)
   ...
   y(k)      = calcQuadraticBezierYFcnXDerivative(x(k), curveParams, 0);
   dydx(k)   = calcQuadraticBezierYFcnXDerivative(x(k), curveParams, 1);
   d2ydx2(k) = calcQuadraticBezierYFcnXDerivative(x(k), curveParams, 2);
   d3ydx3(k) = calcQuadraticBezierYFcnXDerivative(x(k), curveParams, 3);
end
