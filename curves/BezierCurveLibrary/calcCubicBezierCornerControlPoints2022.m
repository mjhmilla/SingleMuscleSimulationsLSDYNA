...
%    135(2), 021005.
%%
function xyPts = calcQuinticBezierCornerControlPoints2021(x0,y0,dydx0,d2ydx20,flag0_zeroD3yDx3,...
                                              x1,y1,dydx1,d2ydx21,flag1_zeroD3yDx3,...
                                              curviness)
%%
...
%      x(0):                        x0
%   dxdu(0):  5*(              x1 - x0)
% d2xdu2(0): 20*(       x2 - 2*x1 + x0)
% d3xdu3(0): 60*(x3 - 3*x2 + 3*x1 - x0)
%
%      y(0):                        y0
%   dydu(0):  5*(              y1 - y0)
% d2ydu2(0): 20*(       y2 - 2*y1 + y0)
% d3ydu3(0): 60*(y3 - 3*y2 + 3*y1 - y0)
%
%
%      x(1):                        x5
%   dxdu(1):  5*(              x4 - x5)
% d2xdu2(1): 20*(       x3 - 2*x4 + x5)
% d3xdu3(1): 60*(x2 - 3*x3 + 3*x4 - x5)
%
%      y(1):                        y5
%   dydu(1):  5*(              y4 - y5)
% d2ydu2(1): 20*(       y3 - 2*y4 + y5)
% d3ydu3(1): 60*(y2 - 3*y3 + 3*y4 - y5)
%
%
% d1ydx1 = dydu/dxdu 
% d2ydx2 = dy2du2/dxdu  - dydu*d2xdu2/(dxdu*dxdu))
% d3ydx3 = dy3du3/dxdu 
%        - dy2du2*d2xdu2/(dxdu*dxdu)  
%        - d2ydu2*d2xdu2/(dxdu*dxdu)) 
%          - dydu*d3xdu3/(dxdu*dxdu)) 
%  +2*dydu*d2xdu2*d2xdu2/(dxdu*dxdu*dxdu))
%
%
% Equations: match d2ydx2
%
% x0 = x(0)
% y0 = y(0)
%
...
xyPts(2,2) = y0 + curviness*(yC1-xyPts(1,2));
...
xyPts(5,2) = xyPts(6,2) + curviness*(yC2-xyPts(6,2));
