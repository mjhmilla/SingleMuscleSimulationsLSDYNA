...
deltaX = min(0.2*(fIso/kIso), 0.2*(xIso-xZero));
...
fiberForceLengthCurve.dydxEnd      = [kZero, kIso];
fiberForceLengthCurve.d2ydx2End    = [0, 0];

fiberForceLengthCurve.integral = [];

%%
% Relax the 2nd derivative between the first two curves
% Here I'm using optimization, but this can be calculated given
% the desired end conditions of the 2 splines.
%%

% idxLeft = 1;
% idxRight= idxLeft+1;
% 
% errFcn = @(arg)calcThirdDerivativeError(arg,idxLeft,...
...
%        ypts(3,idxRight)];
% 
% argMin =[ypts(3,idxLeft); ...
%          ypts(2,idxRight)];
% 
% argMax =[ypts(5,idxLeft);
%          ypts(4,idxRight)];
% 
% 
% err0 = errFcn(arg0);       
% errFcn = @(arg)calcThirdDerivativeError(arg,idxLeft,...
%                   fiberForceLengthCurve,(1/err0));
% 
% 
% [x,fval,exitflag] = fmincon(errFcn, arg0, [], [], [], [], argMin, argMax);
% 
% if(fval < 1)
%   fiberForceLengthCurve.ypts(4,idxLeft)  = x(1,1);
%   fiberForceLengthCurve.ypts(3,idxRight) = x(2,1);  
% end
