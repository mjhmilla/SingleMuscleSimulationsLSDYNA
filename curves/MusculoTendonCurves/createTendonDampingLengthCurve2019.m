...
%    135(2), 021005.
%%
function tendonDampingLengthCurve = ...
          createTendonDampingLengthCurve2019( eIso, kIso, ...
                                        fToe, curviness, ...
                                        computeIntegral, ...
...
%				  curve in it (e.g. 'bicep_tendonDampingLengthCurve') 
...
dydxToeD = 0;
if(flag_enableNumericallyNonZeroGradients==1)
  dydxToeD = smallNumericallyNonZeroNumber;
end
yToeDCtrl  = yToeCtrl/yToe;
dydxDCtrlL = (yToeDCtrl-y0)/(xToeCtrl-x0);
dydxDCtrlR = (yToeD-yToeDCtrl)/(xToe - xToeCtrl);
dydxDCtrl  = (1.25+0.125)*max(dydxDCtrlL,dydxDCtrlR);

p12 = calcQuinticBezierCornerControlPoints(...
              x0,             y0,           dydx0, 0,...
            xToe,          yToeD,          dydxToeD, 0, c);          
          

xpts = [p12(:,1)];
ypts = [p12(:,2)];

tendonDampingLengthCurve.xpts    = xpts;
tendonDampingLengthCurve.ypts    = ypts;

tendonDampingLengthCurve.xEnd         = [x0, xToe];
tendonDampingLengthCurve.yEnd         = [y0, yToeD];
tendonDampingLengthCurve.dydxEnd      = [dydx0, dydxToeD];
tendonDampingLengthCurve.d2ydx2End    = [0, 0];

tendonDampingLengthCurve.integral = [];

if(computeIntegral == 1)
    xScaling = eIso;
    tendonDampingLengthCurve.integral = ...
        createCurveIntegralStructure(tendonDampingLengthCurve, ...
                                     1000,...
                                     1e-12,...
